...
# --- 3c. CAPTURE THIS RELEASE'S BLOCK-CORPUS ROW, BEFORE THE TAG EXISTS ------
#
# Same invariant as the migration row, same window, same fix. The corpus
# ("this build reads every block shape any released version wrote",
# tests/block_corpus.rs) was regenerated by hand after a block shape changed —
# a ritual with no gate — and went stale by ELEVEN releases with every check
# green (2026-08-10 review, finding 2). tests/block_corpus.rs now derives the
# tags it expects from $expectations, so the row appended above makes a
# missing corpus file fail the suite; capturing here is what keeps that check
# green on the release commit itself rather than red on whoever pushes next.
printf 'Capturing the block-corpus row for %s.\n' "$tag"
corpus_row="tests/fixtures/block-corpus/$tag.jsonl"
if ! scripts/capture-block-corpus.sh --current "$tag"; then
  undo_row
  die "capture-block-corpus.sh failed for the release tree; nothing has been tagged.
That is a harness fault — do not release without the corpus row, because
tests/block_corpus.rs will (correctly) go red on the expectations row above."
fi
[ -s "$corpus_row" ] || {
  undo_row
  die "the corpus capture wrote no blocks for $tag, which no release since
v0.2 has been true of. Diagnose the capture before releasing — an absent row
reads as 'this version wrote nothing', which is a claim about the release."
}
printf 'captured corpus row: %s (not yet committed)\n' "$corpus_row"
...
if ! git add "$expectations" "$corpus_row"; then
  undo_row
  die "could not stage the migration row and corpus row; nothing has been tagged"
fi
if ! git commit -q -m "migration matrix and block corpus: the measured rows for $tag" -m \
...
where its absence could not fail until the next release. The block-corpus row
gets the same treatment for the same reason — regenerated by hand, it went
stale by eleven releases with every check green (2026-08-10 review, finding 2).
