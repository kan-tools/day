...
if [[ ${1:-} == --self-test ]]; then
  ...
  reset_fixture; perl -0pi -e 's/- Status: (Draft|Review)/- Kan-claim: bafyrecursive\n- Status: $1/' "$fixture/rfcs/0-rfc-and-adr-process.md"; expect_rejected recursive-publication 'normative RFC bytes contain a claim-CID backlink'
