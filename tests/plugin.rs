...
fn ac7_and_ac8_the_plugin_ships_both_atoms_as_commands() {
    ...
    let markers = [
        ...
        // The premise the whole command turns on: a handoff is a claim about
        // state at the moment it was written, and state moves. A command that
        // reports a handoff as current is copy-paste with extra steps, failing
        // in the direction that matters — a stale handoff read as fresh carries
        // the authority of the record.
        ("commands/handoff.md", "rather than believing it"),
    ];
...
fn command_preambles_exit_zero_even_where_nothing_exists() {
    ...
    assert_eq!(
        checked, 23,
        "expected 23 `!` preamble commands across the shipped command files; found \
