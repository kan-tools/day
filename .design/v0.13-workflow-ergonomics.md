# Release: v0.13 workflow ergonomics, correction round 7
...
This correction supersedes correction-round-6 Plan
`bafyreibpkvpp6rk55qlcppdyopmxhctgza3bjlfdn5n6oj5yai6ui3k53y`. Round 6 made
the `/askme` evidence boundary explicit: deterministic tests check the tracked
interaction contract and absence of automatic writers, while the
preregistered real-harness observation retains before/after scratch-log
evidence for decline and early stop. Before publication, its mirror commit was
rewritten solely to correct an overbroad demonstration trailer so that it names
only the repository-binding test that actually fails under reversion. Round 7
preserves the same requirements, decisions, and evidence layers while issuing
a truthful artifact coordinate for that rewritten history. It also preserves
the policy repair that separates hermetic vector-byte validation from
