# Release: v0.13 workflow ergonomics, correction round 6
...
This correction supersedes correction-round-5 Plan
`bafyreib6apnfa5s7bhvqzvhidkcqnvwc7pol2dl56wl3i7tcrbbjyrhr5y`. Round 5 made
the candidate boundary equal the implementation and adopted the proportionate
instrumentation policy, but AC-6 still assigned real `/askme` decline and stop
behavior to deterministic scratch-log tests even though `/askme` is a
prompt-driven affordance. Round 6 makes that boundary explicit: deterministic
tests check the tracked interaction contract and absence of automatic writers;
the preregistered real-harness observation retains before/after scratch-log
evidence for decline and early stop. It also preserves the policy repair that
separates hermetic vector-byte validation from repository-address resolution.
